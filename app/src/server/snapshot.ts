 ...
 * counts, prices, and Whop object ids, and it must never be serialised to the
 * browser. Only the projection in `./projection` crosses to the client.
 */

import { readOwningAccountId, readPlans, readProductDetail, readProducts } from "./whop-client";

/**
 * How many products get a detail read for their affiliate fields.
 *
 * Deliberately above the top tier boundary (20), so the bucket a district lands
 * in is exact whenever a business has this many products or fewer, and
 * saturates correctly when it has more. A bounded cap keeps a large catalogue
 * from turning one page load into a hundred requests.
 */
...
export type BusinessSnapshot = {
  ...
  capturedAt: number;
  /** False when the API could not be reached. The city renders honestly dark. */
  reachable: boolean;
 ...
 *
 * A failed read is not an error state to the caller: an unreachable API yields
 * `reachable: false` and empty collections, and the city renders as dormant
...
export async function captureSnapshot(env: Env): Promise<BusinessSnapshot> {
  ...
  if (accountId === null) {
    return { accountId: null, capturedAt: Date.now(), reachable: false, products: [], plans: [] };
  }

  const [products, plans] = await Promise.all([readProducts(env, accountId), readPlans(env, accountId)]);

  // Affiliate fields are absent from the list payload, so they are read per
  // product, bounded by the cap above.
  ...
  return {
    ...
    products: products.map((product) => {
      ...
      return {
        ...
        planType: product.default_plan?.plan_type ?? null,
      };
    }),
